classdef LevelDiagram < handle
    ...
    methods (Access = private)
        ...
        function onConceptVisibility(obj, src, conceptIdx)
            ...
            % Mostrar/ocultar sección en panel de información
            visStr = 'on'; if ~visible; visStr = 'off'; end
            if conceptIdx <= numel(obj.panelLabels) && ...
               ~isempty(obj.panelLabels{conceptIdx}) && isvalid(obj.panelLabels{conceptIdx})
                obj.panelLabels{conceptIdx}.Visible = visStr;
            end
            if conceptIdx <= numel(obj.panelButtons) && ~isempty(obj.panelButtons{conceptIdx})
                for b = 1:numel(obj.panelButtons{conceptIdx})
                    btn = obj.panelButtons{conceptIdx}{b};
                    if ~isempty(btn) && isvalid(btn)
                        btn.Visible = visStr;
                    end
                end
            end
            if conceptIdx <= numel(obj.panelTables) && ...
               ~isempty(obj.panelTables{conceptIdx}) && isvalid(obj.panelTables{conceptIdx})
                obj.panelTables{conceptIdx}.Visible = visStr;
            end
        ...
        function selectByZone(obj, xLim, yLim, conceptIdx, ax)
            ...
            % Si el eje es de objetivos, encontrar su índice de columna directamente
            % (evita fallos cuando distintos conceptos tienen etiquetas diferentes)
            axObjIdx = find(cellfun(@(a) ~isempty(a) && isvalid(a) && (a == ax), ...
                                    obj.axesObjectives), 1);
            ...
            for ci = ciList
                ...
                c     = obj.concepts{ci};
                sync  = obj.syncValues{ci};
                % Usar índice de columna si es un eje de objetivos; si no, por etiqueta
                if ~isempty(axObjIdx) && axObjIdx <= c.pfdim
                    xData = c.objectives(:, axObjIdx);
                else
                    xData = obj.getDataByLabel(c, xLabel);
                end
                if isempty(xData); continue; end
